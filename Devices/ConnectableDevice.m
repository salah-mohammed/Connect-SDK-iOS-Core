...
- (void)deviceService:(DeviceService *)service pairingRequiredOfType:(DeviceServicePairingType)pairingType withData:(id)pairingData
{
    if (self.delegate)
    {
        ...
        {
            UIAlertController *pairingDataVC = (UIAlertController*)pairingData;
            if ((pairingType == DeviceServicePairingTypeAirPlayMirroring) && (pairingDataVC.isViewLoaded) == NO){
                UIViewController *vc  = [[[UIApplication sharedApplication] windows] objectAtIndex:0].rootViewController;
                [vc presentViewController:pairingDataVC animated:true completion:nil];
            }
        }
    }
}
